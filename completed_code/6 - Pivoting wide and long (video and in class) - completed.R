...
bob_long %>% 
  group_by(season) |> 
  filter(is_present == 1) |> 
  summarize(mtn_count = sum(str_detect(object, 'mountain')),
            count_possible = n())
...
bob |>
  ...
  summarise(distinct_feature_count = n()) |>
  group_by(season) |>
...
bob |>
  pivot_longer(
    cols = apple_frame:last_col(), #you caould also use last_col() instead of wood_framed
    names_to = 'feature',
    values_to = 'is_in_frame'
  ) |>
  filter(is_in_frame == 1) %>% 
  select(season, feature, is_in_frame) %>% 
  group_by(season, feature) %>% 
  summarize(count = sum(is_in_frame == 1)) %>% 
  pivot_wider(
    names_from = season,
    values_from = count
  )
