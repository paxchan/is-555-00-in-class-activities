...
# show the top 5 tallest overall - using slice_head
df %>% 
  arrange(desc(height)) %>% 
  slice_head(n = 5)
...
# With a sloppy join (i.e., where we don't specify the join logic), we create 
# unexpected results with the two year columns. Notice the mostly empty columns 
# from the `df_planes` tibble... this is because the join is attempting to join 
# on both tailnum and year, and the years mean different things.
df_flights %>% 
  left_join(df_planes) %>% 
  filter(!is.na(engine)) %>%
  glimpse()

# So first we need to explicitly provide the join logic:
df_flights %>% 
  left_join(df_planes, by = join_by(tailnum))

# But that leads to `year.x` and `year.y` in the resulting dataset, since both
# dataframes have a `year` column.

# Here's one strategy: just rename the right-side year on the fly to something 
# useful. Name conflict solved.
df_flights %>% 
